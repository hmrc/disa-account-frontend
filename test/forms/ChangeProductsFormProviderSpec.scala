...
class ChangeProductsFormProviderSpec extends BaseUnitSpec {

  val form: Form[Set[IsaProduct]] = new ChangeProductsFormProvider()()

  "ChangeProductsFormProvider" should {

    "bind every supported product" in
      IsaProduct.values.zipWithIndex.foreach { case (product, index) =>
    ...
    "bind multiple products" in {
      val selected = IsaProduct.values.take(2)
    ...
    "return the required error when no product is selected" in {
      ...
      result.errors.map(_.message) should contain("changeProducts.error.required")
    }
    ...
    "fill previously selected products" in {
      val selected = IsaProduct.values.takeRight(2).toSet
