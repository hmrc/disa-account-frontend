...
case object PeerToPeerPlatformPage extends GuardedPage with PageWithAnswers[String] {

  def canBeAccessedWith(answers: Answers): Boolean = answers.innovativeFinancialProducts.exists(
  ...
  def saveAnswerAndHandleDependents(request: DataRequest[_], newAnswer: String): SessionUpdates = {
    ...
    existingUpdates.copy(p2pPlatform = Assign(newAnswer))
  }
}
