...
case object FinancialOrganisationPage extends PageWithAnswers[Set[FinancialOrganisation]] {

  def saveAnswerAndHandleDependents(
    request: DataRequest[_],
    newAnswer: Set[FinancialOrganisation]
  ): SessionUpdates = {
    val existingUpdates = request.sessionAnswers.fold(SessionUpdates())(_.updates)
    val orderedAnswer   = FinancialOrganisation.values.filter(newAnswer.contains)

    existingUpdates.copy(financialOrganisation = Assign(orderedAnswer))
  }
}
