...
case object OrganisationEmailAddressPage extends PageWithAnswers[String] {
  ...
  def saveAnswerAndHandleDependents(request: DataRequest[_], newAnswer: String): SessionUpdates = {
    ...
    // A fresh verification code is always sent on submit, so any previous verification no longer holds.
    existingUpdates.copy(organisationEmailAddress = Assign(newAnswer), organisationEmailVerified = Clear)
  }
}
