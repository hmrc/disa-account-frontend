...
object LiaisonOfficerNameSummary {
  def row(officer: LiaisonOfficer)(implicit messages: Messages): Option[SummaryListRow] =
    officer.fullName.map { name =>
      SummaryListRow(
        key = Key(Text(messages("liaisonOfficerName.checkYourAnswersLabel"))),
        value = Value(Text(name)),
        actions = Some(
          Actions(
            items = Seq(
              ActionItem(
                href = LiaisonOfficerNameController.onPageLoad(Some(officer.id), CheckMode).url,
                content = Text(messages("site.change")),
                visuallyHiddenText = Some(messages("liaisonOfficerName.change.hidden"))
              )
            )
          )
        )
