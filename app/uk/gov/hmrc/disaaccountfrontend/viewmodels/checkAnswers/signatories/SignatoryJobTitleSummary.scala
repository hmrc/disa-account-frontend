...
object SignatoryJobTitleSummary {
  ...
  def row(signatory: Signatory)(implicit messages: Messages): Option[SummaryListRow] =
    signatory.jobTitle.map { jobTitle =>
      SummaryListRow(
        key = Key(Text(messages("signatoryJobTitle.checkYourAnswersLabel"))),
        value = Value(Text(jobTitle)),
        actions = Some(
          Actions(
            items = Seq(
              ActionItem(
                href = SignatoryJobTitleController.onPageLoad(signatory.id, CheckMode).url,
                content = Text(messages("site.change")),
                visuallyHiddenText = Some(messages("signatoryJobTitle.change.hidden"))
              )
            )
          )
        )
