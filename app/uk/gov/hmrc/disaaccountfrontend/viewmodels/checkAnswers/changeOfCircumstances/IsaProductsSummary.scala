...
object IsaProductsSummary {
  ...
  def productName(product: IsaProduct)(implicit messages: Messages): String =
    messages(s"changeProducts.${product.toString}")

  def row(answers: Answers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.isaProducts.filter(_.nonEmpty).map { products =>
      val lines = IsaProduct.values.filter(products.contains).map(productName)

      SummaryListRow(
        key = Key(Text(messages("isaProducts.checkYourAnswersLabel"))),
        value = Value(HtmlContent(lines.map(line => HtmlFormat.escape(line).toString).mkString("<br>"))),
        actions = Some(
          Actions(
            items = Seq(
              ActionItem(
                href = ChangeProductsController.onPageLoad().url,
                content = Text(messages("site.change")),
                visuallyHiddenText = Some(messages("isaProducts.change.hidden"))
              )
            )
          )
        )
