...
object OrganisationTelephoneNumberSummary {
  ...
  def row(answers: Answers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.organisationTelephoneNumber.map { telephoneNumber =>
      SummaryListRow(
        key = Key(Text(messages("organisationTelephoneNumber.checkYourAnswersLabel"))),
        value = Value(Text(telephoneNumber)),
        actions = Some(
          Actions(
            items = Seq(
              ActionItem(
                href = OrganisationTelephoneNumberController.onPageLoad().url,
                content = Text(messages("site.change")),
                visuallyHiddenText = Some(messages("organisationTelephoneNumber.change.hidden"))
              )
            )
          )
        )
