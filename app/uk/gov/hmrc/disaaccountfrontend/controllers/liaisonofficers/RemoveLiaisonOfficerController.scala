...
import uk.gov.hmrc.disaaccountfrontend.models.pages.liaisonofficers.RemoveLiaisonOfficerPage
import uk.gov.hmrc.disaaccountfrontend.views.html.liaisonofficers.RemoveLiaisonOfficerView
        ...
        form
          ...
          .fold(
            ...
            value => {
              ...
              val sessionUpdates = getSessionUpdates(RemoveLiaisonOfficerPage.apply(id), value)

              userAnswersRepository.set(UserAnswers(id = request.sessionId, updates = sessionUpdates)).map { _ =>
                Redirect(nextPage(RemoveLiaisonOfficerPage(id), sessionUpdates))
              }
