...
import uk.gov.hmrc.disaaccountfrontend.views.html.liaisonofficers.LiaisonOfficerCommunicationView
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class LiaisonOfficerCommunicationController @Inject() (
  ...
  view: LiaisonOfficerCommunicationView
)(implicit ec: ExecutionContext)
    extends PageController(navigator)
    with FrontendBaseController
    with I18nSupport {

  private val form: Form[Set[LiaisonOfficerCommunication]] = formProvider()

  private def page(id: String): LiaisonOfficerCommunicationPage =
    LiaisonOfficerCommunicationPage(id)

  private def pageAction(currentPage: LiaisonOfficerCommunicationPage) =
  ...
  def onPageLoad(id: String, mode: Mode): Action[AnyContent] = {
    ...
    pageAction(currentPage) { implicit request =>
      ...
      val preparedForm   = liaisonOfficer.fold(form)(officer => form.fill(officer.communication))
