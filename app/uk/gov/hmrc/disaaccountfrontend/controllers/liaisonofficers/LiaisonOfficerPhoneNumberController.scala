...
import uk.gov.hmrc.disaaccountfrontend.models.pages.liaisonofficers.LiaisonOfficerPhoneNumberPage
...
import uk.gov.hmrc.disaaccountfrontend.views.html.liaisonofficers.LiaisonOfficerPhoneNumberView
...
class LiaisonOfficerPhoneNumberController @Inject() (
  ...
  private val form: Form[String] = formProvider("liaisonOfficerPhoneNumber")

  private def page(id: String): LiaisonOfficerPhoneNumberPage =
    LiaisonOfficerPhoneNumberPage(id)

  private def pageAction(currentPage: LiaisonOfficerPhoneNumberPage) =
  ...
  def onPageLoad(id: String, mode: Mode): Action[AnyContent] = {
    ...
    pageAction(currentPage) { implicit request =>
      ...
      val preparedForm   = liaisonOfficer.flatMap(_.phoneNumber).fold(form)(form.fill)
